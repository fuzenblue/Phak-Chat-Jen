                ...
                <div className="bg-white w-full rounded-3xl shadow-xl shadow-green-900/5 p-8 border border-white">
                    ...
                    {tab === 'register' && (
                        <form onSubmit={handleRegister} className="space-y-5">
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-gray-700 ml-1">ประเภทผู้ใช้</label>
                                <div className="grid grid-cols-2 gap-3">
                                    <label className={`flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all ${
                                        ...
                                        <span className="font-semibold text-sm">ลูกค้า</span>
                                    </label>
                                    <label className={`flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all ${
                                        regRole === 'merchant'
                                            ? 'border-green-500 bg-green-50 text-green-700'
                                        ...
                                        <input
                                            ...
                                            name="regRole"
                                            value="merchant"
                                            checked={regRole === 'merchant'}
                                            onChange={() => setRegRole('merchant')}
                                            className="hidden"
                                        />
                                        <span className="material-symbols-outlined text-[22px]">storefront</span>
                                        <span className="font-semibold text-sm">ร้านค้า</span>
                                    </label>
                                </div>
                            </div>

                            <button type="submit" disabled={loading} className="w-full bg-green-500 hover:bg-green-600 active:scale-[0.98] text-white font-bold rounded-2xl py-4 shadow-lg shadow-green-200 transition-all flex items-center justify-center">
                                {loading ? <span className="animate-spin material-symbols-outlined">progress_activity</span> : regRole === 'merchant' ? "สมัครสมาชิกและตั้งค่าร้าน" : "สมัครสมาชิก"}
                            </button>
                        </form>
                    )}
                </div>
